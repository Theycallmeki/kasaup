...
<template>
  <div class="page">
    <div class="page-header">
      ...
      <div class="stats-row">
        ...
        <div class="stat-pill active-stat">
          <span>Upcoming</span>
          <b>{{appointmentStore.appointments.filter((a: any) => a.status === 'approved' || a.status === 'confirmed').length
            }}</b>
        </div>
      </div>
    </div>
    ...
    <div class="appointments-layout">
            ...
            <div class="card-body">
              <div class="cb-top">
                ...
                <div class="cb-badges">
                  ...
                  <span v-if="app.customer_latitude && app.customer_longitude || app.customer_address"
              ...
              <div class="cust-info">
                <div class="ci-avatar">
                  <img v-if="app.customer_profile_image" :src="app.customer_profile_image" alt=""
                    class="ci-avatar-img" />
              ...
              <div class="cb-bottom">
                <div class="cb-actions">
                  <button v-if="app.status === 'pending'" class="abtn abtn-green"
                    @click="approve(app.id)">Approve</button>
                  <button v-if="app.status === 'approved' || app.status === 'confirmed'" class="abtn abtn-blue"
                    @click="complete(app.id)">Complete</button>
                  ...
                  <button v-if="app.status === 'pending' || app.status === 'approved' || app.status === 'confirmed'"
                    class="abtn abtn-red" @click="cancel(app.id)">Cancel</button>
                </div>

                <button v-if="app.customer_latitude && app.customer_longitude || app.customer_address"
                  class="map-toggle-btn" :class="{ open: selectedApp?.id === app.id }" @click="selectApp(app)">
      ...
      <CustomerLocationMapCard :show="Boolean(selectedApp)" :lat="selectedApp?.customer_latitude ?? null"
        :lng="selectedApp?.customer_longitude ?? null" :customerName="selectedApp?.customer_name"
        :address="selectedApp?.customer_address" :providerLat="authStore.user?.latitude"

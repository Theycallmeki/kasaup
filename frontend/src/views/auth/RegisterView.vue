      ...
      <template v-else>
        ...
        <form @submit.prevent="register">
          <div class="field-wrap">
            <input v-model="full_name" class="field" :class="{ 'field-error': fieldErrors.full_name }"
              placeholder="Full Name" required @blur="fieldErrors.full_name = validateFullName(full_name) || ''" />
            <span v-if="fieldErrors.full_name" class="field-hint-error">{{ fieldErrors.full_name }}</span>
          </div>
          <div class="field-wrap">
            <input v-model="email" class="field" :class="{ 'field-error': fieldErrors.email }" type="email"
              placeholder="Email" required @blur="fieldErrors.email = validateEmail(email) || ''" />
            <span v-if="fieldErrors.email" class="field-hint-error">{{ fieldErrors.email }}</span>
          </div>
          <div class="field-wrap">
            <div class="password-wrapper">
              <input v-model="password" class="field pr-field" :class="{ 'field-error': fieldErrors.password }"
                :type="showPassword ? 'text' : 'password'" placeholder="Password (min 8 chars)" required
